...
import { FuelChartClient } from "./fuel-chart-client";
...
export async function FuelLogSummaryWidget({ userId }: FuelLogWidgetsProps) {
  const [vehicles, fuelLogs] = await Promise.all([
    VehicleService.getAll(userId),
    FuelLogService.getAll(userId),
  ]);

  const totalCost = fuelLogs.reduce((sum, log) => sum + log.total_cost, 0);
  const totalVolume = fuelLogs.reduce((sum, log) => sum + log.volume, 0);
  const avgEfficiency =
    totalVolume > 0 ? (totalCost / totalVolume).toFixed(2) : "0";

  return (
    <WidgetContainer
      title="Fuel Summary"
      description="Your fuel consumption overview"
    >
...
export async function FuelLogChartWidget({ userId }: FuelLogWidgetsProps) {
  ...
  return (
    <WidgetContainer
      ...
      size="large"
    >
      <FuelChartClient data={chartData} />
    </WidgetContainer>
  );
}

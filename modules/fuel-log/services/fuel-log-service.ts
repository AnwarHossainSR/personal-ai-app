...
export class FuelLogService {
  ...
  static async create(
    ...
    if (data.vehicle_id && data.odometer) {
      ...
      if (lastEntry && data.odometer < lastEntry.odometer) {
        throw new Error(
          "Odometer reading cannot be less than the previous entry"
        );
      }
    }
  ...
  static async update(
    ...
    userId: string,
    data: Partial<IFuelLog>
  ): Promise<IFuelLog | null> {
    await dbConnect();
    return FuelLog.findOneAndUpdate({ _id: id, user_id: userId }, data, {
      new: true,
    });
  }

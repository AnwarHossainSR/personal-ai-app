...
async function seedDatabase() {
  ...
  try {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      fuelLogs.push({
        vehicle_id: vehicleIds[Math.floor(Math.random() * vehicleIds.length)].toString(),
        user_id: userResult.insertedId.toString(),
        date,
        ...
        station: ["Shell", "Exxon", "BP", "Chevron", "Mobil"][Math.floor(Math.random() * 5)],
        created_at: new Date(),
        updated_at: new Date(),
      })
    }
    ...
    const serviceLogs = []
    const serviceTypes = ["oil_change", "tire_rotation", "brake_service", "tune_up", "repair"]
